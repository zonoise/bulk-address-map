...
import { buildPastePreview, getPasteRecords, looksLikeDelimitedTable, parseDelimitedText } from "./addressPaste";
...
describe("住所貼り付け解析: CSVや表データから住所列を取り出す", () => {
  ...
  it("CSVでは住所列を優先し、名称や電話番号を混ぜずに抽出する", () => {
    ...
    expect(selected.records).toEqual([
      { address: "東京都新宿区西新宿2-8-1", name: "都庁" },
      { address: "大阪市北区中之島1-3-20", name: "大阪市役所" },
      { address: "札幌市中央区北1条西2丁目", name: "札幌市役所" },
    ]);
    expect(getPasteRecords(preview, preview.selectedKey, preview.selectedLabelKey)).toEqual([
      { address: "東京都新宿区西新宿2-8-1", name: "都庁" },
      { address: "大阪市北区中之島1-3-20", name: "大阪市役所" },
      { address: "札幌市中央区北1条西2丁目", name: "札幌市役所" },
    ]);
  ...
  it("HTMLテーブルでは見出しから住所列と施設名を判断して抽出する", () => {
    const html = `
      <table>
  ...
  it("見出しなしの2列データではラベル列を選んで住所に紐づけられる", () => {
    const text = [
      "福岡県\t福岡県福岡市博多区東公園7番7号",
      "佐賀県\t佐賀県佐賀市城内1丁目1番59号",
      "長崎県\t長崎県長崎市尾上町3番1号",
    ].join("\n");

    const preview = buildPastePreview({ html: "", text });
    const addressColumn = preview.columns.find((column) => column.label === "列2");
    const labelColumn = preview.labelColumns.find((column) => column.label === "列1");

    expect(addressColumn.values).toEqual([
      "福岡県福岡市博多区東公園7番7号",
      "佐賀県佐賀市城内1丁目1番59号",
      "長崎県長崎市尾上町3番1号",
    ]);
    expect(getPasteRecords(preview, addressColumn.key, labelColumn.key)).toEqual([
      { address: "福岡県福岡市博多区東公園7番7号", name: "福岡県" },
      { address: "佐賀県佐賀市城内1丁目1番59号", name: "佐賀県" },
      { address: "長崎県長崎市尾上町3番1号", name: "長崎県" },
    ]);
  });
  ...
  it("都道府県・市区町村・番地のように分割された住所列は結合して抽出する", () => {
    ...
    expect(selected.records).toEqual([
      { address: "東京都 新宿区西新宿 2-8-1", name: "都庁" },
      { address: "大阪府 大阪市北区中之島 1-3-20", name: "大阪市役所" },
    ]);
  });
